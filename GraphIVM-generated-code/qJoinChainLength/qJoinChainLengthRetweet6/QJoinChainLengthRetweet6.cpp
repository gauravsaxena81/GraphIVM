...
void addRetweet4Tuple(int& r3TweetIdR4RetweetTweetId, int& r4TweetIdR5RetweetTweetId );
void addRetweet5Tuple(int& r4TweetIdR5RetweetTweetId, int& r5UserIdFUserId );
void addFollowerTuple(int& r5UserIdFUserId, int& followerId, bool doBfs );
...
Retweet5JoinTupleMap* retweet5JoinTupleMap = new Retweet5JoinTupleMap;
...
Retweet4Retweet5VertexValueMap* retweet4Retweet5VertexValueMap = new Retweet4Retweet5VertexValueMap;
Retweet5FollowerVertexValueMap* retweet5FollowerVertexValueMap = new Retweet5FollowerVertexValueMap;
	...
	try {//PT
		...
		cout<<"Start Retweet Table Load"<<endl;
	      for (pqxx::result::const_iterator c = RU.begin(); c != RU.end(); ++c) {
			...
			addRetweet4Tuple(r, t); 
			addRetweet5Tuple(r, u); 
	      }
...
void bfsRetweet5Diff(Retweet5JoinTupleEntry* retweet5Jt ) {}

void bfsFollowerDiff(FollowerJoinTupleEntry* followerJt , FollowerProjectedTupleEntry* followerPt) {
	for(Retweet5JoinTupleEntry* i3 : followerJt->retweet5FollowerVertexValue->retweet5JoinTupleList) {
		for(Retweet4JoinTupleEntry* i2 : i3->retweet4Retweet5VertexValue->retweet4JoinTupleList) {
					...
					for(Retweet1JoinTupleEntry* j : i1->retweet1Retweet2VertexValue->retweet1JoinTupleList) {
						for(int index1 = 0; index1 < j->retweet1ProjectedTupleMap->capacity; index1++) {
							int max = followerPt->count
									...
									* i3->retweet5ProjectedTupleMap->retweet5ProjectedTupleEntryArray[0]->count;
...
void addRetweet4Tuple(int& r3TweetIdR4RetweetTweetId, int& r4TweetIdR5RetweetTweetId ) {
	Retweet4JoinTupleEntry* jt = putRetweet4JoinTupleIfAbsent(retweet4JoinTupleMap, r3TweetIdR4RetweetTweetId, r4TweetIdR5RetweetTweetId , retweet3Retweet4VertexValueMap, retweet4Retweet5VertexValueMap );
	Retweet4ProjectedTupleEntry* pt = putRetweet4ProjectedTuple(jt->retweet4ProjectedTupleMap);
	bfsRetweet4Diff(jt );
}

void addRetweet5Tuple(int& r4TweetIdR5RetweetTweetId, int& r5UserIdFUserId ) {
	Retweet5JoinTupleEntry* jt = putRetweet5JoinTupleIfAbsent(retweet5JoinTupleMap, r4TweetIdR5RetweetTweetId, r5UserIdFUserId , retweet4Retweet5VertexValueMap, retweet5FollowerVertexValueMap );
	Retweet5ProjectedTupleEntry* pt = putRetweet5ProjectedTuple(jt->retweet5ProjectedTupleMap);
	bfsRetweet5Diff(jt );
}

void addFollowerTuple(int& r5UserIdFUserId, int& followerId, bool doBfs ) {
	FollowerJoinTupleEntry* jt = putFollowerJoinTupleIfAbsent(followerJoinTupleMap, r5UserIdFUserId , retweet5FollowerVertexValueMap );
