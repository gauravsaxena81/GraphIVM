...
static void increaseArraySize(Follower2Follower3VertexValueMap* follower2Follower3VertexValueMap) {
	...
	Follower2Follower3VertexValueEntry** newVertexValueEntryArray = new Follower2Follower3VertexValueEntry*[newcapacity]();
	for(int i = 0; i < follower2Follower3VertexValueMap->capacity; i++) {
		...
		if(p) {
			...
			h = h & (newcapacity - 1);
			do {
                Follower2Follower3VertexValueEntry* nextP = p->next;
	...
	follower2Follower3VertexValueMap->follower2Follower3VertexValueEntryArray = newVertexValueEntryArray;
