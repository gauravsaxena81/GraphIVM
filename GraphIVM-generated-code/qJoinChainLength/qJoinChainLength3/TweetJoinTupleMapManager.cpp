...
static void ensureSize(TweetJoinTupleMap* tweetJoinTupleMap, int& follower2IdTweetUserId);
//%2 - vertex value attribute values 
static void increaseArraySize(TweetJoinTupleMap* tweetJoinTupleMap, int& follower2IdTweetUserId);
static TweetJoinTupleEntry* next();
//%1$1s - join attributes
static void addToVertexValues(int& follower2IdTweetUserId , TweetJoinTupleEntry* p, Follower2TweetVertexValueMap* follower2TweetVertexValueMap);
...
__inline static void ensureSize(TweetJoinTupleMap* tweetJoinTupleMap, int& follower2IdTweetUserId) {
    if(tweetJoinTupleMap->size >= tweetJoinTupleMap->capacity * 0.8) {
        increaseArraySize(tweetJoinTupleMap, follower2IdTweetUserId );
    }
}
//%2 - join attributes
static void increaseArraySize(TweetJoinTupleMap* tweetJoinTupleMap, int& follower2IdTweetUserId) {
...
TweetJoinTupleEntry* putTweetJoinTupleIfAbsent(TweetJoinTupleMap* tweetJoinTupleMap, int& follower2IdTweetUserId, Follower2TweetVertexValueMap* follower2TweetVertexValueMap ) {
    ...
    //%2 - join attributes
    ensureSize(tweetJoinTupleMap, follower2IdTweetUserId );
...
__inline static void addToVertexValues(int& follower2IdTweetUserId , TweetJoinTupleEntry* p, Follower2TweetVertexValueMap* follower2TweetVertexValueMap) {
    //%4 - vertex attribute values
...
//}
//%1 - join attributes
inline static void hashCode(int&h, int& follower2IdTweetUserId) {
