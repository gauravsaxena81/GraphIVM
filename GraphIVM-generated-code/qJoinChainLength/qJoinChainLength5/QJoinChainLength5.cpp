...
void addFollower4Tuple(int& follower3IdUser4Id, int& follower4IdTweetUserId );
void addTweetTuple(int& follower4IdTweetUserId, int& tweetId, bool doBfs );
...
Follower4TweetVertexValueMap* follower4TweetVertexValueMap = new Follower4TweetVertexValueMap;
...
void bfsTweetDiff(TweetJoinTupleEntry* tweetJt, TweetProjectedTupleEntry* tweetPt) {
	for(Follower4JoinTupleEntry* j1 : tweetJt->follower4TweetVertexValue->follower4JoinTupleList) {
...
void addFollower4Tuple(int& follower3IdUser4Id, int& follower4IdTweetUserId ) {
    //%3 - join attributes, %4 - vertex value maps
	Follower4JoinTupleEntry* jt = putFollower4JoinTupleIfAbsent(follower4JoinTupleMap, follower3IdUser4Id, follower4IdTweetUserId , follower3Follower4VertexValueMap, follower4TweetVertexValueMap );
	//%3 - projected attributes
	bfsFollower4Diff(jt );
}
...
void addTweetTuple(int& follower4IdTweetUserId, int& tweetId, bool doBfs ) {
    //%3 - join attributes, %4 - vertex value maps
	TweetJoinTupleEntry* jt = putTweetJoinTupleIfAbsent(tweetJoinTupleMap, follower4IdTweetUserId , follower4TweetVertexValueMap );
	//%3 - projected attributes
