...
static void increaseArraySize(Follower3Follower4VertexValueMap* follower3Follower4VertexValueMap) {
	...
	Follower3Follower4VertexValueEntry** newVertexValueEntryArray = new Follower3Follower4VertexValueEntry*[newcapacity]();
	for(int i = 0; i < follower3Follower4VertexValueMap->capacity; i++) {
		...
		if(p) {
			...
			h = h & (newcapacity - 1);
			do {
                Follower3Follower4VertexValueEntry* nextP = p->next;
	...
	follower3Follower4VertexValueMap->follower3Follower4VertexValueEntryArray = newVertexValueEntryArray;
