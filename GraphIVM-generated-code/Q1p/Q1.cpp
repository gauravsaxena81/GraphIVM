...
int main() {
	try {//PT
		...
		//for(int i = 0; i < 22831; i++) {
		//	int x = 10000 + i;
		//	int y = 10000 + 2 * i;
		//	std::string c2 = "1992-01-01";
		//	addTweetTuple(x, y, c2, true);
		//}
	      for (pqxx::result::const_iterator c = R.begin(); c != R.end(); ++c) {
			if(c[3].as<int>() == -1)
			{
		...
		for(int index1 = 0; index1 < i->userProjectedTupleMap->capacity; index1++) {
			...
			while(currentP) {
				for(int m = 0; m < currentP->count; m++)
					api(currentP->userId, userIdTweetUserId, tweetId, tweetDate);
...
void addTweetTuple(int& userIdTweetUserId, int& tweetId, std::string& tweetDate, bool doBfs ) {
	...
	if(doBfs)
//		api(userIdTweetUserId, userIdTweetUserId, tweetId, tweetDate);
...
void deleteTweetTuple(int& userIdTweetUserId, int& tweetId, std::string& tweetDate, bool doBfs ) {
	...
	if(doBfs)
		bfsTweetDiff((*jtp), userIdTweetUserId, tweetId, tweetDate);
