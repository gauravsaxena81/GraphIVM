...
#include <stdlib.h>
#include <iostream>
inline static void hashCode(int&h, int& userId ) __attribute__((always_inline));
...
__inline static void hashCode(int& h, int& userId ) {
    h ^=  userId + 0x9e3779b9 + (h<<6) + (h>>2);
}
UserProjectedTupleEntry* putUserProjectedTuple(UserProjectedTupleMap* userProjectedTupleMap, int& userId ) {
	int h = 31;
	hashCode(h
	, userId
	); 
