...
public class FastJoinDeltaManagerQ4Aggregation {
    private enum ALIAS {
    ...
    private static final List<String> PROJECTED_ATTRIBUTES_LIST_TWEET = Arrays.asList("t_user_id", "tweet_id");
   ...
   @BeforeClass
    public static void setUp() throws InstantiationException, IllegalAccessException, ClassNotFoundException {
        ...
        tweetDefiniton.setProjectedAttributeNamesList(PROJECTED_ATTRIBUTES_LIST_TWEET);
        ...
        try(Connection connection = DriverManager.getConnection("jdbc:postgresql://localhost:5432/bsmagen", "gaurav", "")){
            ...
            aggregationSqlTableLoader.loadTable(connection.createStatement().executeQuery("select A.user_id as t_user_id, A.tweet_id from tweet A --where tweet_id = 4374")
    ...
    public void insertProjectedTupleTest() {
        ...
        for(int k = 0; k < 15; k++) {
            ...
            for(int i = 0; i < MAX; i++) {
                insertFollower(17, X + i, FastAggregationManager, viewMaintainer, aggregatedAttributes, true);
            }
            long t = (System.nanoTime() - tm)  / 1000000;
            for(int i = 0; i < MAX; i++) {
                deleteFollower(17, X + i, FastAggregationManager, viewMaintainer, aggregatedAttributes, false);
            }
    ...
    , ClassNotFoundException {
        ...
        for(int k = 0; k < 15; k++) {
            ...
            long t = (System.nanoTime() - tm)  / 1000000;
            for(int i = 0; i < MAX; i++) {
                deleteFollower(17, X + i, FastAggregationManager, viewMaintainer, aggregatedAttributes, true);
            }
            System.out.println(TestUtil.getTestName() + t + "/" + MAX);
        }
    }
...
//    }
    ...
    , ClassNotFoundException {
        ...
        for(int j = 0; j < fanout; j++) {
            insertTweet(X, X + j, fastAggregationManager);
            insertReTweet1(X + j, X + j, fastAggregationManager);
        }
        for(int i = 0; i < MAX; i++) {
            for(int j = 0; j < fanout; j++)
                insertReTweet2(X + j, X + i, fastAggregationManager, viewMaintainer, aggregatedAttributes, false);
        }
        for(int k = 0; k < 15; k++) {
            long tm = System.nanoTime();
            for(int i = 0; i < MAX; i++) {
    ...
    private void insertTweet(int user, int tweet, FastAggregationManager aggregationManager) {
      ...
      projectedAttributeValues.add(user);//user-id present
      ...
      aggregationManager.insertTupleWithoutReturn(cntTable, vertexValues, projectedAttributeValues
              , TYPE.JOIN_TUPLE, 0, "tweet", cntTable.getTableDefinition(), PROJECTED_ATTRIBUTES_LIST_TWEET);
    }
    @Test
    public void deleteJoinedTuple1To1Test() throws InstantiationException, IllegalAccessException
    , ClassNotFoundException {
    ...
    private void insertReTweet1(int retweetTweetId, int tweetId, FastAggregationManager aggregationManager) {
        JoinTuple tuple = new JoinTuple(TYPE.GROUPED_TUPLE, 1);
        tuple.setTableDefinition(cntTable.getTableDefinition());
        
        VertexValue[] vertexValues = new VertexValue[2];
        VertexValue<ALIAS> vertexValue = new VertexValue<ALIAS>();
        vertexValue.setAlias(ALIAS.ALIASTWEETID);
        List<Object> joinAttributeValues = new ArrayList<>();
        vertexValue.setAttributeValues(joinAttributeValues);
        ...
        List<Object> projectedAttributeValues = new ArrayList<>();
        aggregationManager.insertTupleWithoutReturn(cntTable, vertexValues, projectedAttributeValues
                        , TYPE.GROUPED_TUPLE, 1, "retweet1", cntTable.getTableDefinition(), null);
    }
