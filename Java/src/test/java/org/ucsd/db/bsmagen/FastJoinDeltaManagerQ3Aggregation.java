...
public class FastJoinDeltaManagerQ3Aggregation {
    private enum ALIAS {
    ...
    public static void setUp() throws InstantiationException, IllegalAccessException, ClassNotFoundException, SQLException {
        ...
        try(//Connection connection = DriverManager.getConnection("jdbc:h2:~/bsmagen;QUERY_CACHE_SIZE=0", "sa", "");
            ...
            aggregationSqlTableLoader.loadTable(connection.createStatement().executeQuery("select user_id as u_user_id from users-- where user_id = 2"), userDefiniton, 
            ...
            aggregationSqlTableLoader.loadTable(connection.createStatement().executeQuery("select user_id as t_user_id, tweet_id "
                    + "as t_tweet_id from tweet-- where tweet_id = 729"), tweetDefinition, 
            ...
            aggregationSqlTableLoader.loadTable(connection.createStatement().executeQuery("select tweet_id as r_tweet_id"
                    + ", retweet_tweet_id from retweet-- where retweet_tweet_id = 729"), retweetDefinition, 
    ...
    public void scanTest() {
        ...
        for(int i = 0; i < 10; i++) {
            ...
            fastAggregationManager.scan(cntTable);
    ...
    , ClassNotFoundException {
        ...
        for(int k = 0; k < 20; k++) {
            ...
            long tm = System.nanoTime();
            for(int i = 0; i < MAX; i++) {
                insertReTweet(729, X + i, fastAggregationManager, viewMaintainer, aggregateAttributes, true);
            }
            System.out.println(TestUtil.getTestName() + (System.nanoTime() - tm)  / 1000000 + "/" + MAX);
            for(int i = 0; i < MAX; i++) {
                deleteReTweet(729, X + i, fastAggregationManager, viewMaintainer, aggregateAttributes, false);
            }
        }
    }
    @Test
    ...
    , ClassNotFoundException {
        ...
        for(int k = 0; k < 20; k++) {
            for(int i = 0; i < MAX; i++) {
                insertReTweet(729, X + i, fastAggregationManager, viewMaintainer, aggregateAttributes, false);
            }
            long tm = System.nanoTime();
            for(int i = 0; i < MAX; i++) {
                deleteReTweet(729, X + i, fastAggregationManager, viewMaintainer, aggregateAttributes, true);
            }
            System.out.println(TestUtil.getTestName() + (System.nanoTime() - tm)  / 1000000 + "/" + MAX);
        }
    }
...
//                viewMaintainer.applyTuple(i, aggregateAttributes, -1);
    ...
    private void deleteUser(int user, FastAggregationManager fastAggregationManager) {
        VertexValue<ALIAS>[] vertexValues = new VertexValue[1];
        VertexValue<ALIAS> vertexValue = new VertexValue<ALIAS>();
        vertexValue.setAlias(ALIAS.ALIASUSERID);
        List<Object> joinAttributeValues = new ArrayList<>();
        ...
        List<Object> projectedAttributeValues = new ArrayList<>();
        projectedAttributeValues.add(user);
        fastAggregationManager.deleteTupleWithoutReturn(cntTable, vertexValues, projectedAttributeValues, TYPE.JOIN_TUPLE, 0);
    }
}
