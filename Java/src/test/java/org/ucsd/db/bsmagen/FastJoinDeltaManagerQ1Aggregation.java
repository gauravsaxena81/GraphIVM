...
public class FastJoinDeltaManagerQ1Aggregation {
    private enum ALIAS {
    ...
    public static void setUp() throws InstantiationException, IllegalAccessException, ClassNotFoundException {
        ...
        try(Connection connection = DriverManager.getConnection("jdbc:postgresql://localhost:5432/bsmagen", "gaurav", "")) {
            sqlTableLoader.loadTable(connection.createStatement().executeQuery("select user_id as u_user_id from users where user_id = 2"), userDefiniton, 
                        tableDefinition, cntTableAndIndices, 0);
            sqlTableLoader.loadTable(connection.createStatement().executeQuery("select user_id as t_user_id from tweet where user_id = 2"), tweetDefinition, 
...
//    }
    ...
    private void insertTweet(int user, int tweet, FastAggregationManager fastAggregationManager
        ...
        //VertexValue<ALIAS>[] vertexValues = new VertexValue[]{FastViewUtil.getOldVertexValue(ALIAS.ALIASUSERID, user)};
...
//                viewMaintainer.applyTuple(i, aggregatedAttributes, 1);
    ...
    private void deleteTweet(int user, int tweet, FastAggregationManager fastAggregationManager
        ...
        vertexValue.setAttributeValues(joinAttributeValues);
        vertexValues[0] = vertexValue;
        //vertexValues[0] = FastViewUtil.getOldVertexValue(ALIAS.ALIASUSERID, user);
...
//                viewMaintainer.applyTuple(i, aggregatedAttributes, -1);
    ...
    , ClassNotFoundException {
        ...
        for(int k = 0; k < 15; k++) {
            long tm = System.nanoTime();
            for(int i = 0; i < MAX; i++) {
                insertTweet(x + i, x + i, fastJoinDeltaManager, aggregatedAttributes, true);
            }
            System.out.println(TestUtil.getTestName() + (System.nanoTime() - tm)  / 1000000 + "/" + MAX);
            for(int i = 0; i < MAX; i++) {
                deleteTweet(x + i, x + i, fastJoinDeltaManager, aggregatedAttributes, false);
            }
        }
    }
