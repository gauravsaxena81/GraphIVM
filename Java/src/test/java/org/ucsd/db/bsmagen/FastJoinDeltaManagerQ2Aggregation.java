...
public class FastJoinDeltaManagerQ2Aggregation {
    private enum ALIAS {
        ALIASUSERID
    }
    ...
    , ClassNotFoundException {
        ...
        for(int k = 0; k < 25; k++) {
            ...
            for(int i = 0; i < MAX; i++) {
                deleteTweet(100, X + i, fastJoinDeltaManager, aggregatedAttributes, false);
            }
        }
    }
    @Test
    ...
    private void insertTweet(int user, int tweet, FastAggregationManager aggregationManager
        ...
        if(doReturn) {
            List<Object[]> t = 
                            aggregationManager.insertTuple(cntTable, vertexValues, projectedAttributeValues
