...
public class FastJoinDeltaManager1MQ4 {
   ...
   @BeforeClass
    ...
    , ClassNotFoundException {
        ...
        for(int k = 0; k < 11; k++) {
            ...
            for(int i = 0; i < MAX; i++) {
                insertFollower(1175996, X + i, fastJoinDeltaManager, viewMaintainer, true);
            }
            long t = (System.nanoTime() - tm)  / 1000000;
            System.out.println(TestUtil.getTestName() + t + "/" + MAX);
            for(int i = 0; i < MAX; i++) {
                deleteFollower(1175996, X + i, fastJoinDeltaManager, viewMaintainer, false);
            }
        }
    }
    ...
    , ClassNotFoundException {
        ...
        for(int k = 0; k < 10; k++) {
            for(int i = 0; i < MAX; i++) {
                insertFollower(1175996, X + i, fastJoinDeltaManager, viewMaintainer, false);
            }
            long tm = System.nanoTime();
            for(int i = 0; i < MAX; i++) {
                deleteFollower(1175996, X + i, fastJoinDeltaManager, viewMaintainer, true);
            }
            long t = (System.nanoTime() - tm)  / 1000000;
            System.out.println(TestUtil.getTestName() + t + "/" + MAX);
        }
...
//    }
    ...
    , ClassNotFoundException {
        ...
        for(int j = 0; j < fanout; j++)
            insertReTweet1(336759, X + j, fastJoinDeltaManager);
        for(int i = 0; i < MAX; i++) {
            for(int j = 0; j < fanout; j++)
    ...
    , ClassNotFoundException {
        ...
        for(int k = 0; k < 15; k++) {
            for(int i = 0; i < MAX; i++) {
                insertFollower(X + i, 2 * X + i, fastJoinDeltaManager, viewMaintainer, false);                
            }
            long tm = System.nanoTime();
            for(int i = 0; i < MAX; i++) {
                deleteFollower(X + i, 2 * X + i, fastJoinDeltaManager, viewMaintainer, true);
            }
            System.out.println(TestUtil.getTestName() + (System.nanoTime() - tm)  / 1000000 + "/" + MAX);
        }
    }
