   ...
   @BeforeClass
    ...
    , ClassNotFoundException {
        ...
        for(int k = 0; k < 11; k++) {
            ...
            for(int i = 0; i < MAX; i++) {
                insertFollower(384827, X + i, fastJoinDeltaManager, viewMaintainer, true);
            }
            long t = (System.nanoTime() - tm)  / 1000000;
            System.out.println(TestUtil.getTestName() + t + "/" + MAX);
            for(int i = 0; i < MAX; i++) {
                deleteFollower(384827, X + i, fastJoinDeltaManager, viewMaintainer, false);
            }
        }
    }
    ...
    , ClassNotFoundException {
        ...
        for(int k = 0; k < 10; k++) {
            for(int i = 0; i < MAX; i++) {
                insertFollower(384827, X + i, fastJoinDeltaManager, viewMaintainer, false);
            }
            long tm = System.nanoTime();
            for(int i = 0; i < MAX; i++) {
                deleteFollower(384827, X + i, fastJoinDeltaManager, viewMaintainer, true);
            }
            long t = (System.nanoTime() - tm)  / 1000000;
            System.out.println(TestUtil.getTestName() + t + "/" + MAX);
        }
