...
//		CustomProgram_2(int argc = 0, char* argv[] = 0) : Program(argc,argv) {}
        void process_streams() {
			...
			int MAX = 50000;
			long X = 1000000000;
			//relation_id_t r = get_relation_id("TWEET");
			
			//long *u = new long(X);
			//data.on_insert_USERS(*u);
			const auto start_time = std::chrono::steady_clock::now();
			for( long i = 1; i <= MAX; ++i ) {
				//ev_args.clear();
				//ev_args.push_back(new long(X));
				//ev_args.push_back(new long(X + i));
				//ev_args.push_back(new STRING_TYPE("2992-01-01"));
				//event_t ev( insert_tuple, r, 0, ev_args);
				//Program::process_stream_event(ev);
				const long *u = new long(17);
				const long *f = new long(X + i);
				data.on_insert_FOLLOWER(*u, *f);
			}
            ...
            /*for( long i = 1; i <= MAX; ++i ) {
                ...
                ev_args.push_back(new long(2));
                ev_args.push_back(new long(X + i));
				ev_args.push_back(new STRING_TYPE("2992-01-01"));
                event_t ev( delete_tuple, get_relation_id("TWEET"), 0, ev_args);
                Program::process_stream_event(ev);
            }*/
        }
    };
