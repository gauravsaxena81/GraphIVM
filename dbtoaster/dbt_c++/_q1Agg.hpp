...
namespace dbtoaster {
  ...
  struct data_t : tlq_t{
    ...
    /* Trigger functions for stream relations */
     void on_insert_TWEET(const long tweet_user_id, const long tweet_tweet_id, const STRING_TYPE& tweet_tweet_date) {
      ...
      BEGIN_TRIGGER(ivc_stats,"insert_TWEET")
        //r[0] = rr;
      {  //++tN;
        api.useDiffQ1Agg(tweet_user_id, __SQL_COUNT_AGGREGATE_1_mTWEET1.getValueOrDefault(se2.modify(tweet_user_id)));
    ...
    std::vector<void*> on_delete_TWEET(const long tweet_user_id, const long tweet_tweet_id, const STRING_TYPE& tweet_tweet_date) {
      BEGIN_TRIGGER(exec_stats,"delete_TWEET")
      BEGIN_TRIGGER(ivc_stats,"delete_TWEET")
      {  //++tN;
        api.useDiffQ1Agg(tweet_user_id, (__SQL_COUNT_AGGREGATE_1_mTWEET1.getValueOrDefault(se5.modify(tweet_user_id)) * -1L));
  ...
  class Q1Agg : public Program
  {
    public:
      Q1Agg(int argc = 0, char* argv[] = 0) : Program(argc,argv) {
      }
  };
