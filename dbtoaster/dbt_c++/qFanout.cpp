    ...
    public:
        void process_streams() {
			...
			if(which == 1) {
				...
				for(long j = 1; j <= fanout; j++) {
					data.on_insert_FOLLOWER(2 * X + j, j);
				}
                                for( long i = 1; i <= MAX; ++i ) {
					for(long j = 1; j <= fanout; j++) {
						data.on_insert_FOLLOWER(X + i, 2 * X + j);
					}
				}
                                auto start_time = std::chrono::steady_clock::now();
				STRING_TYPE *s = new STRING_TYPE("2992-01-01");
                                for( long i = 1; i <= MAX; ++i ) {
                                        data.on_insert_TWEET(X + i, X + i, *s);
                                }
				...
				//COUNT_map m = data.get_COUNT();
				//start_time = std::chrono::steady_clock::now();
				//int x = 0;
				//static int y = 0;	
				//m.foreach([x] (const COUNT_entry& e) {
					//y += api.useDiff(e.A_FOLLOWER_ID, e.Z_TWEET_ID);
				//});
				//double overhead = std::chrono::duration_cast<std::chrono::milliseconds> (std::chrono::steady_clock::now() - start_time).count();
				//cout<< "scan:"<< y << ":" << overhead <<endl;
			}
