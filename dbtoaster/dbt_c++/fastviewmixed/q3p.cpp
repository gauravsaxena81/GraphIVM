		...
		try {//PT
			pqxx::connection C("dbname=bsmagen1m user=gaurav password=gaurav \
			 ...
			 return;
		      ...
		      std::string sql = "select * from (select user_id, tweet_id, tweet_date, -1 as retweet_tweet_id from tweet T union select * from retweet order by tweet_id desc limit 100000) A order by tweet_id asc";
			...
			start_time = std::chrono::steady_clock::now();
		      for (pqxx::result::const_iterator c = R.begin(); c != R.end(); ++c) {
				...
				if(retweet_tweet_id > -1)
					...
					//data.on_insert_RETWEET(c[0].as<long>(), c[1].as<long>(),*s, 2);
				else
